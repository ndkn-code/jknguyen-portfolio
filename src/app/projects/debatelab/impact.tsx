...
const stats = [
  {
    ...
    detail: "Full relational schema",
  },
  {
    value: "4",
    label: "AI Services",
    detail: "Gemini, Groq, Deepgram STT/TTS",
  },
  {
    value: "12",
    label: "Scoring Sub-Categories",
    detail: "Comprehensive debate analysis",
  },
  {
    value: "16",
    label: "Achievements",
    detail: "Gamification system",
  },
];
          ...
          {stats.map((stat, i) => (
            <FadeUp key={i} delay={0.1 * i}>
              <div>
                ...
                />
                <div className="text-xs uppercase tracking-[0.15em] font-semibold text-muted-foreground mb-1">
                  {stat.label}
                </div>
        ...
        <FadeUp delay={0.3}>
          <div className="bg-white dark:bg-card p-5 sm:p-8 md:p-12 rounded-xl border border-border mb-12 text-left max-w-3xl mx-auto">
            ...
            <div className="space-y-4">
              {[
                "Full-stack edtech product shipped solo, from database design to production deployment",
                "Real-time audio pipeline: WebSocket STT + AI analysis + TTS playback in a single session flow",
                "Two distinct AI models working together: Gemini for structured analysis, Groq for conversational coaching",
                "Complete Vietnamese localization with Gen Z tone, not just translated strings",
                "Gamification that drives retention: XP, streaks, achievements, and selectable profile titles",
                "API cost tracking built into the platform via Supabase api_usage table",
              ].map((item, i) => (
                <motion.div
                  key={i}
